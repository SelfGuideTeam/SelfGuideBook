  ...
  <v-container>
    <v-row>
      <v-col cols="12" align="center">
        ...
        <v-card width="1200" tile flat outlined="true">
          ...
          <v-card tile flat>
            <v-row>
              <v-col cols="12" sm="4" align="center">{{ items.writer }}</v-col>
              ...
              <v-col cols="12" sm="4" align="center">
                <a class="ml-6" @click="fileDownload(items)">
                  {{
                  items.filenames.length > 20
                  ? items.filenames.substring(0, 20) + ".."
                  : items.filenames
                  }}
                </a>
          ...
          <v-toolbar flat color="transparent">
            <v-spacer></v-spacer>
            <v-btn class="ml-3" color="secondary" tile outlined @click="goToBoard">게시판으로 글 이동</v-btn>
            <v-btn class="ml-3" color="secondary" tile outlined @click="modifyGuideBook()">수정</v-btn>
            <v-btn class="ml-3" color="secondary" tile outlined @click="GuideBook_Dialog()">삭제</v-btn>
            <v-btn class="ml-3" color="secondary" tile outlined @click="myGuideBookPage()">뒤로</v-btn>
          </v-toolbar>
        </v-card>
      </v-col>
    </v-row>
    <v-dialog v-model="dialog_GuideBook_Delete" max-width="300">
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="#90CAF9" class="white--text" @click="GuideBook_Delete()">삭제</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-container>
        ...
        this.$firebase
          ...
          .then(url => {
            ...
            xhr.onload = event => {
              ...
              this.file = new File([blob], this.items.filenames, {
                type: blob.type
              });
              resolve();
            };

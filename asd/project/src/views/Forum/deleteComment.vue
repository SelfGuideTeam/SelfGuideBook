<template>
  <v-card>
    <v-card-text class="pt-5">삭제하시겠습니까?</v-card-text>
    <v-card-actions>
      <v-spacer></v-spacer>
      <v-btn
        color="green darken-1"
        text
        @click="cancelDeleteComment(val.id1, val.id2); sendDialog2();"
      >취소</v-btn>
      <v-btn color="green darken-1" text @click="deleteComment(val.id1, val.id2); sendDialog2();">확인</v-btn>
    </v-card-actions>
  </v-card>
</template>
<script>
